...
export default Retailer_demand = ({navigation}) => {
    ...
    const Com = rows.length?<FlatList data = {rows} renderItem = {renderItem} keyExtractor={item=>item.id}/>:<View style={{paddingTop:100, alignItems:'center'}}><Image style={{width:300, height:200}} source={require('./assets/retailer.jpg')}/>
    ...
    <View style={{height:height, backgroundColor:'white'}}>
        <LinearGradient colors={['#28c380', '#1b9862', '#00693c']}  start={{x: 0, y: 0}} end={{x: 0, y: 1}} style={{width:'100%', height:'12%', flexDirection:'row', justifyContent:'center', alignItems:'center'}}>
            <Text style={{color:'white', fontWeight:'500', fontSize:17}}>Enlist Your Demand</Text>
        </LinearGradient>
        ...
        <View style={{position:'absolute', borderTopWidth:1.3, borderTopColor:'rgb(234, 234, 234)', padding: 17, backgroundColor:'white', bottom:0, ...styles.center, flexDirection:'row', width:'100%', justifyContent:'space-between'}}>
                ...
                <TouchableOpacity style={{...styles.center}} onPress={()=>{setColor1(c2);setColor2(c1);setColor3(c2);setColor4(c2);setColor5(c2);navigation.navigate('Enlist Your Demand');}}>

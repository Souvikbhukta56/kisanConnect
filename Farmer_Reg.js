...
export default Farmer_Reg = ({ navigation }) => {
    ...
    const cr = () => {
        ...
        get(child(dbref, 'Farmers')).then((snapshot) => {
          // if(!snapshot.val()){
            storeAsync({Id:nam.replace( /\s/g, '')+'@kc', Password:pass, Name:nam, Number:ph});
            setModalDis(false);
            storeData('Farmers', 'Accounts', {Name: nam, Phone:ph, Address:addr, Gmail:mail, Id: nam.replace( /\s/g, '')+'@kc', Password:pass});
            navigation.navigate('KishanConnect');
          // }
          // else{
          //   alert("Account already exists");
          //   setNam('');setPass('');setId('');setCpass('');
    ...
    return (
        <SafeAreaView style={{backgroundColor:'white', height:height, width:width}}>
            ...
            <View style={{position:'absolute', top:220}}>
              <View style={{width:width, ...styles.center}}><Text style={{fontWeight:'300' , color:'white'}}>Farmer Registration</Text></View>
              ...
              <View style={{backgroundColor:'white', elevation:5, marginTop:20, height:50, width:280, borderRadius:30, flexDirection:'row', marginLeft:width/2-140}}>
                ...
                <TextInput style={{...styles.textinput}} placeholder='Kishan full name..' placeholderTextColor={'rgb(151, 147, 147)'} value={nam} onChangeText={text => setNam(text)}/>
              </View>
              ...
              <View style={{backgroundColor:'white', elevation:5, marginTop:20, height:50, width:280, borderRadius:30, flexDirection:'row', marginLeft:width/2-140}}>
                ...
                <TextInput style={{...styles.textinput}} placeholder='Verified KCC id (optional)' placeholderTextColor={'rgb(151, 147, 147)'} value={id} onChangeText={text => setId(text)}/>
              </View>
              <View style={{backgroundColor:'white', elevation:5, marginTop:20, height:50, width:280, borderRadius:30, flexDirection:'row', marginLeft:width/2-140}}>
                <View style={{backgroundColor:'#28c380', marginTop:10,marginLeft:12, height:30, width:30, borderRadius:15, justifyContent:'center', alignItems:'center'}}>
